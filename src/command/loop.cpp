...
void Loop::Exec(const dpp::slashcommand_t &event) {
	...
	if (loop_type_par.index() == 0) {
		event.reply(Bragi::Get(event.command.guild_id)->LoopCommand(event.command.channel_id));
		return;
	}
	...
	if (loop_type_str == "t")
		...
		return event.reply(Bragi::Get(event.command.guild_id)->LoopCommand(event.command.channel_id, Bragi::LoopType::PLAYLIST));
	else
		return event.reply(Bragi::Get(event.command.guild_id)->LoopCommand(event.command.channel_id, Bragi::LoopType::DISABLED));
}

void Loop::Exec(const dpp::message_create_t &event, const std::string &argument) {
	/* If the argument is empty */
	if (argument.empty()) {
		event.send(Bragi::Get(event.msg.guild_id)->LoopCommand(event.msg.channel_id));
		return;
	}
