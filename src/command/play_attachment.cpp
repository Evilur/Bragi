...
void PlayAttachment::Exec(dpp::cluster &bot, const dpp::slashcommand_t &event) {
	AttachmentTrack track(event.command.channel_id, nullptr);
	
	try { event.reply(Exec(bot, event.command.usr.id, event.command.guild_id, event.command.channel_id, &track)); }
