...
OpusSender::OpusSender(dpp::discord_voice_client *const voiceclient,
                       const unsigned char speed_percent) :
    _voiceclient(voiceclient),
    _resampler_output_freq(FREQ * 100 / speed_percent) {
    _encoder = opus_encoder_create(FREQ, CHANNELS, OPUS_APPLICATION_AUDIO,
                                   nullptr);
    _resampler = speex_resampler_init(2, RESAMPLER_INPUT_FREQ,
                                      _resampler_output_freq, 10, nullptr);
}
...
void OpusSender::SendData(const short *in_left, const short *in_right,
                          const unsigned short in_size) {
    /* Get the output pcm array size with some extra space for overflow protection */
    const unsigned int out_size =
        in_size * _resampler_output_freq / RESAMPLER_INPUT_FREQ + 64;
    ...
    /* Resample input data and put the result to the output arrays */
    {
        ...
        speex_resampler_process_int(_resampler, 0, in_left, &in_left_size,
                                    out_left, &out_left_size);
        speex_resampler_process_int(_resampler, 1, in_right, &in_right_size,
                                    out_right, &out_right_size);
    }
